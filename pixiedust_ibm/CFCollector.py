...
class CFCollector:
    def __init__(self, config=None):
        ...
        if config is None or ((config.get('api_token') is None) and (config.get('ibmid') is None or config.get('password') is None)):
            raise Exception('You must specify an IBM Cloud user api_token or an ibmid and password.')

        if config.get('api_token') is not None:
            self.id = 'apikey'
            self.password = config.get('api_token') 
        else:
            self.id = config.get('ibmid')  
